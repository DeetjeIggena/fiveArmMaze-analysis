...
% containing data for single trials
...
for group = 1:gN
    ...
    for p = 1: participantLength
        ...
        for f = 1:numel(files)
            ...
            % get time-out
            fam(n).timeOut = fam_timeOut(88, fam(n).time); 
            ...
            if goalAlley == fam(n).start
                goalAlleyEntry      = 0;
                goalAlleyTime       = 0;
                goalAlleyRel        = 0;
            else
                goalAlleyEntry      = zone_alley_entry(goalAlley);
                goalAlleyTime       = zone_alley_time(goalAlley);
                goalAlleyRel        = zone_alley_rel(goalAlley);
            end
            ...
            fam(n).pathScore          = fam_pathScore(zone_alleyOut_abs,...
                zone_alleyIn_abs,...
...
fam_writeToXLSX(fam, resultFolder)
datestr(now)
close all
clear variables